...
void PhysicsEngine::AddSoftBodyMeshObject(SoftbodyMeshes* objs)
{
    softbodyMeshObjects.push_back(objs);
}

void PhysicsEngine::RemoveSoftBodyMeshObject(SoftbodyMeshes* objs)
{
    std::vector<SoftbodyMeshes*> ::iterator it = std::find(softbodyMeshObjects.begin(),
        softbodyMeshObjects.end(), objs);

    if (it != softbodyMeshObjects.end())
    {

        softbodyMeshObjects.erase(it);
    }
    else
    {
        std::cout << "Softbody object not found in the vector." << std::endl;
